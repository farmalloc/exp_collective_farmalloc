...
def main():
    ...
    data = {}
    data["dfs"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_dfs_btree.log"), dtype=dtype, ndmin=2)
    data["vEB"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_veb_btree.log"), dtype=dtype, ndmin=2)
    data["local"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_local_btree.log"), dtype=dtype, ndmin=2)
    data["local+dfs"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_local+dfs_btree.log"), dtype=dtype, ndmin=2)
    data["local+vEB"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_local+veb_btree.log"), dtype=dtype, ndmin=2)
    data["hint"] = np.loadtxt(os.path.join(log_dir, "kvs_benchmark_with_hint_btree.log"), dtype=dtype, ndmin=2)
    ...
    for zipf_skewness, update_ratio, subfigure_idx in ((0.8, 0.05, "a"), (0.8, 0.5, "b"), (1.3, 0.05, "c"), (1.3, 0.5, "d")):
        ...
        fig.savefig(
            os.path.join(
                os.path.dirname(__file__), f"../charts/figure16{subfigure_idx}.pdf"
            ),
            bbox_inches="tight",
        )

        legendfig.legend(map(lambda x: x[0], lines), labels, ncol=6).get_frame().set_alpha(1.0)
        legendfig.savefig(
            os.path.join(
                os.path.dirname(__file__), f"../charts/figure16_legend.pdf"
            ),
            bbox_inches="tight",
        )
